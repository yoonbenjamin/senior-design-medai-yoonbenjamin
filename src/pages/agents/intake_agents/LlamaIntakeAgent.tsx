...
const LlamaIntakeAgent: React.FC = () => {
    const [feedback, setFeedback] = useState(''); // For storing user feedback
    const [improvedSummary, setImprovedSummary] = useState<string | null>(null); // Store the improved summary
    const [isFeedbackLoading, setIsFeedbackLoading] = useState(false); // Loading state for feedback submission
    const { isLoggedIn } = useAuth();
    const [selectedFile, setSelectedFile] = useState<File | null>(null);
    const [summaries, setSummaries] = useState<{ summary1: string; summary2: string } | null>(null); // Store both summaries
    const [displayedSummaries, setDisplayedSummaries] = useState<{ summary1: string; summary2: string }>({ summary1: '', summary2: '' });
    const [selectedSummary, setSelectedSummary] = useState<'summary1' | 'summary2' | null>('summary1'); // Default to summary1
    ...
    const handleSendFeedback = async () => {
        ...
        try {
            ...
            const response = await fetch('http://127.0.0.1:5001/improve-summary', {
                method: 'POST',
                body: formData,
            });
            ...
            setIsFeedbackLoading(false); // Stop loading
        }
    };
    ...
    const handleUpload = async () => {
        if (selectedFile) {
            ...
            try {
                const response = await fetch('http://127.0.0.1:5001/summarize-pdf', {
                    method: 'POST',
                    body: formData,
                });
                ...
                const data = await response.json();
                setSummaries({ summary1: cleanText(data.summary1), summary2: cleanText(data.summary2) });
                setSelectedSummary('summary1');
            } catch (error) {
                ...
                {isLoggedIn ? (
                    <>
                        <div className="flex justify-between items-center mb-8">
                            ...
                            <h2 className="text-3xl font-semibold">Patient Intake Agent (Llama)</h2>
                        </div>
                        ...
                        <div className="mt-8">
                            ...
                            {summaries && (
                                <div>
                                    ...
                                    >
                                        {isFeedbackLoading ? 'Submitting Feedback...' : 'Submit Feedback & Improve Results'}
                                    </button>
                                    {improvedSummary && (
...
export default LlamaIntakeAgent;
